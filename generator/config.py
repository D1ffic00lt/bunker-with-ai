...
default_template = """Ты - генератор профилей игроков для игры "Бункер"
...
Вам повезло, вы оказались перед входом в спасательный бункер, в котором можно пережить самый опасный период. 
Но попасть в бункер смогут не все – а лишь половина из вас! За несколько раундов игроки решают, кого НЕ берут в бункер. 
Попавшие в бункер – выживут, чтобы затем возродить цивилизацию. Игроки получают несколько случайных карт характеристик: 
пол и возраст, профессия, здоровье, хобби и др. Постепенно вы раскрываете свои карты, знакомитесь друг с другом и 
принимаете решения, кто и насколько будет полезен для выживания и восстановления жизни после выхода из бункера. 
...
- Ты можешь использовать максимум 1024 символа
"""
...
Но попасть в бункер смогут не все – а лишь половина из вас! За несколько раундов игроки решают, кого НЕ берут в бункер. 
Попавшие в бункер – выживут, чтобы затем возродить цивилизацию. Игроки получают несколько случайных карт характеристик: 
пол и возраст, профессия, здоровье, хобби и др. 
...
result_template = fr"""{default_template}
...
- распиши с участием каждого человека
- генерируй ОБЯЗАТЕЛЬНО ПО ШАБЛОНУ ИНАЧЕ ВСЁ СЛОМАЕТСЯ
- в ответ давай ТОЛЬКО json объект
...
not_bunker_result_template = fr"""{default_template}
Твоя задача - придумать финал игры для игроков, не попавших в бункер 
(как они справились с проблемой и самой катострофой), 
основываясь на сценарии, бункере и оставшихся игроках,
ответ дай по шаблону {result_template_json}
...
player_template = fr"""{default_template}
...
- старайся выдавать программистов ОЧЕНЬ редко

здоровье должно ОБЯЗАТЕЛЬНО быть уникальным, вот список уже использованных заболеваний: <healths>
профессии должны ОБЯЗАТЕЛЬНО быть уникальными, вот список уже использованных профессий: <professions>
"""
...
phobias = [
    'Агорафобия', 'Айхмофобия', 'Акрофобия', 'Агризоофобия',
    'Альгофобия (алгофобия)', 'Аматофобия', 'Аррхенфобия', 'Ежефобия',
    'Астрофобия', 'Бактрахофобия', 'Арахнофобия', 'Баллистофобия',
    'Батофобия', 'Беленофобия', 'Библиофобия', 'Богифобия (фазмофобия)',
    'Ботанофобия (ботонофобия)', 'Бронтофобия (кераунофобия)',
    ...
    'Спектрофобия', 'Тетрафобия', 'Токофобия', 'Технофобия',
    'Фобофобия', 'Эротофобия', 'Птеранофобия', 'Книдофобия',
